...
character o(2)*3,A*11,mm(12)*3,ICMEDATE(3)*10,ICMEHOUR(3)*4,o1*1,OUT*7,Class*5,Polar*2,ABBA*20
integer ICMED(9999,3,3),MONTH,DAY,ICMEnum,CHECKICME(3),CHECKMC(2),grbgI,DOYMM(3000,366,0:25),DOYDD(3000,366,0:25)

open(1,file='MC_DATE_ZHANG.txt')
...
dd(12)=dd(11)+30
    ...
    if (j .lt. 2000) then
    write(A,'(A5,I2.2,A4)') 'CR-H-',j-1900,'.DAT'
    else
    write(A,'(A5,I2.2,A4)') 'CR-H-',j-2000,'.DAT'
    endif
...
write(OUT,'(A4,i3.3)') 'FFFR',j
open(200,file=OUT)

write(200,'(A6,1x,A4,2(a1,A2),1x,A2,A3,1x,A3,3(5x,A3),6(1x,A7),3x,A7,5(1x,A7),5(1x,A6))') ' MCnum','YEAR','/','MM','/',&
'DD','HH',':00','DOY',&
' Int_CR','X11','Y11','Z11',' Bscala','  Bmagn','     Bx','     By','     Bz',' SWtemp',' SWdens','SWspeed','FlPress',&
'PlasmB','Dst-ind','ICMESC','CME_ST','CME_EN','MC_ST','MC_EN'
...
d(2,2)=99
	...
	enddo
      ...
      do l=d(1,1),d(1,1)
      HN=0
	do k=d(1,4)-1,d(2,4)+2
	if (k .eq. d(1,4)-5) then
	   n1=nint(h(1)+1)
	   n2=24
	else
	    if (k .eq. d(2,4)+5) then
	     n1=1
	     n2=nint(h(2)+1)
	    else
	     n1=1
	...
	HN=HN+1
	    ...
	    if ((l .eq. d(1,1)) .and. (k .eq. d(1,4)) .and. (n+1 .eq. nint(h(1)+1))) then
	    write(ABBA,'(A20)') 'o'
	    else
	    endif
	    
	    if ((l .eq. d(2,1)) .and. (k .eq. d(2,4)) .and. (n+1 .eq. nint(h(2)+1))) then
	    write(ABBA,'(A20)') 'o'
	    else
	    endif
	    
	    write(101,'(f6.2,1x,I4.4,2(a1,I2.2),a1,I2.2,A3,I4,9(1x,f7.2),1x,f9.0,5(1x,f7.2),1(1x,A20))') code,l,'/',DOYMM(l,k,n),&
	    '/',DOYDD(l,k,n),' ',n,':00',k,x(1,l,k,n)/1000.,x(2,l,k,n)/1000.,x(3,l,k,n)/1000.,x(4,l,k,n)/1000.,&
	    (omnix(l,k,n,i),i=1,11),ABBA!(CHECKICME(i),i=1,3),(CHECKMC(i),i=1,2)
	    ...
	    write(200,'(f6.2,1x,I4.4,2(a1,I2.2),a1,I2.2,A3,I4,9(1x,f7.2),1x,f9.0,5(1x,f7.2),1(1x,A20))') code,l,'/',DOYMM(l,k,n),&
	    '/',DOYDD(l,k,n),' ',n,':00',k,x(1,l,k,n)/1000.,x(3,l,k,n)/1000.,x(4,l,k,n)/1000.,x(2,l,k,n)/1000.,&
	    (omnix(l,k,n,i),i=1,11),ABBA!(CHECKICME(i),i=1,3),(CHECKMC(i),i=1,2)
	  enddo
	enddo
